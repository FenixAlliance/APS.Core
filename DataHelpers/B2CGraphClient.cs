...
namespace FenixAlliance.Data.Access.Helpers.AADB2C
{
    public class B2CGraphClient : IB2CGraphClient
    {
        ...
        public async Task<string> GetGroupByObjectId(string objectId)
        {
            return await SendGraphGetRequest("/groups/" + objectId, null);
        }
        public async Task<string> GetMemberOf(string objectId)
        {
            return await SendGraphGetMemberRequest("/users/" + objectId);
        }
        ...
        public async Task<string> GetGroupMembersByObjectId(string query)
        {
            return await SendGraphGetGroupMemembersRequest("/groups/", query);
        }
        ...
        public async Task<string> SendGraphGetGroupMemembersRequest(string api, string query)
        {
            // First, use ADAL to acquire a token using the app's identity (the credential)
            // The first parameter is the resource we want an access_token for; in this case, the Graph API.
            AuthenticationResult result = await authContext.AcquireTokenAsync("https://graph.windows.net", credential);
        ...
        public async Task<string> SendGraphGetMemberRequest(string api)
        {
            // First, use ADAL to acquire a token using the app's identity (the credential)
            // The first parameter is the resource we want an access_token for; in this case, the Graph API.
            AuthenticationResult result = await authContext.AcquireTokenAsync("https://graph.windows.net", credential);
        ...
        public async Task<string> GetGroups(string api)
        {
            // First, use ADAL to acquire a token using the app's identity (the credential)
            // The first parameter is the resource we want an access_token for; in this case, the Graph API.
            AuthenticationResult result = await authContext.AcquireTokenAsync("https://graph.windows.net", credential);
